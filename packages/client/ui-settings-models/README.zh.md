...
Local Harness 使用与模型提供商无关的界面文案，模型通过 API 凭据或本地服务配置接入。

“本地 / 自定义 API”提供 Ollama（`http://127.0.0.1:11434/v1`）和 LM Studio（`http://127.0.0.1:1234/v1`）预设。单独启动服务后，获取模型列表或填写模型 ID，再创建提供商。地址可修改。无密钥复选框显式启用无需认证的兼容服务；填写的密钥优先使用，并通过凭据服务保存。本地预设默认上下文为 32,768 token、输出为 4,096 token，请将模型限制设为与服务端一致。工具执行需要支持工具调用的模型。
