...
export const en = {
  nav: 'Models',
  title: 'Models',
  intro: 'Connect local model servers or configure your cloud providers.',
  edit: 'Edit',
  ...
  addCustom: 'Local / custom API',
  localPreset: 'Connection preset',
  localNoAuth: 'This service accepts requests without an API key',
  localOllama: 'Ollama',
  localLmStudio: 'LM Studio',
  localCustom: 'Custom service',
  localOnDevice: 'Local inference',
  localCustomHint: 'OpenAI / Anthropic',
  localSetupHint: 'Start your model server, then fetch its models below. No API key is needed unless your server requires one. Set each model’s context limit to match the server. Tools require a model with tool calling support.',
  ...
  creating: 'Creating\u2026',
  welcomeTitle: 'Welcome to Local Harness',
  welcomeBody: 'Connect a model API or a local Ollama / LM Studio service in Settings → Models. No account is required. Your conversations, tools, and workspaces are managed from this application.',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  addMode: '添加方式',
  addCatalog: '第三方模型提供商',
  addCustom: '本地 / 自定义 API',
  localPreset: '连接方式',
  localNoAuth: '此服务允许不使用 API 密钥连接',
  localOllama: 'Ollama',
  localLmStudio: 'LM Studio',
  localCustom: '自定义服务',
  localOnDevice: '本地推理',
  localCustomHint: 'OpenAI / Anthropic',
  localSetupHint: '启动模型服务后，在下方获取模型列表。服务未启用认证时，无需填写 API 密钥。请将模型上下文长度设为与服务端一致；使用工具需选择支持工具调用的模型。',
  ...
  create: '创建提供商',
  creating: '创建中\u2026',
  welcomeTitle: '欢迎使用 Local Harness',
  welcomeBody: '在设置 → 模型中连接模型 API，或接入本地 Ollama / LM Studio 服务，无需登录账号。在这里统一管理对话、工具和工作区。',
  welcomeContinue: '继续',
