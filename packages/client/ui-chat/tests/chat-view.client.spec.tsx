...
describe('ChatView', () => {
  ...
  it('renders Host-pending steering at the flow tail and hands off to the durable node', withClock(2_000, () => {
    ...
    expect(turnProcessControl(view.container)?.textContent).toBe('正在处理，用时2秒')
  ...
  it('omits the System prompt through Turn completion and process expansion', withClock(4_000, () => {
    ...
    const view = render(<h.ChatView {...h.props} />)
    expect(renderedFlowKinds(view.container)).toEqual(['user', 'turn-process'])
    expect(turnProcessControl(view.container)?.textContent).toBe('正在处理，用时4秒')
  ...
  it('keeps a live Turn expanded and folds it once at turn/end', withClock(3_000, () => {
    ...
    const view = render(<h.ChatView {...h.props} />)
    const liveToggle = view.getByRole('button', { name: '正在处理，用时3秒' }) as HTMLButtonElement
  ...
  it('hands running calls to a live Tool group', withClock(2_000, () => {
    ...
    const toggle = view.getByRole('button', { name: '正在处理，用时2秒' }) as HTMLButtonElement
    expect(toggle.disabled).toBe(true)
    expect(toggle.getAttribute('aria-expanded')).toBe('true')
  }))
  ...
  it('the running clock uses turn/start, ignores steering, and stays out of the live region', withClock(126_000, () => {
    ...
    const toggle = view.getByRole('button', { name: '正在处理，用时2分5秒' }) as HTMLButtonElement
    expect(toggle).toBe(turnProcessControl(view.container))
    expect(toggle.disabled).toBe(true)
    expect(status.textContent).toBe('正在处理')
    ...
    expect(toggle.textContent).toBe('正在处理，用时2分5秒')
    act(() => { vi.advanceTimersByTime(2_000) })
    expect(toggle.textContent).toBe('正在处理，用时2分7秒')
    ...
    act(() => { vi.advanceTimersByTime(1_000) })
    expect(toggle.textContent).toBe('正在处理，用时2分8秒')
    expect(status.textContent).toBe('正在处理')
    view.unmount()
  ...
  it('the running clock reads hours once the turn passes an hour', withClock(3_600_000, () => {
    ...
    const view = render(<h.ChatView {...h.props} />)
    const toggle = view.getByRole('button', { name: '正在处理，用时59分59秒' })
    act(() => { vi.advanceTimersByTime(1_000) })
    expect(toggle.textContent).toBe('正在处理，用时1小时00分0秒')
    act(() => { vi.advanceTimersByTime(303_000) })
    expect(toggle.textContent).toBe('正在处理，用时1小时05分3秒')
    expect(view.getByRole('status').textContent).toBe('正在处理')
    view.unmount()
    expect(vi.getTimerCount()).toBe(0)
  }))
