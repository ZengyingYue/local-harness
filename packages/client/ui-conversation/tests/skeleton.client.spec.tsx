...
describe('Hero chrome', () => {
  it('renders the orbital introduction through the hero locale seat', () => {
    const renderSlot = vi.fn<HeroShellProps['renderSlot']>(() => null)
    const view = render(<HeroShell t={makeTranslate(en, commonEn)} renderSlot={renderSlot} />)
    expect(view.getByText('Make contact. Go beyond.')).toBeTruthy()
    expect(view.getByText('LOCAL HARNESS / ORBITAL STATION')).toBeTruthy()
    ...
    expect(brandMarkOwner.size).toBe(26)
...
describe('ConversationRoot resident composer', () => {
  ...
  it('hero phase: keeps sidebar controls accessible while hiding conversation chrome', () => {
    ...
    expect(b.view.getByText('向未知，发出信号')).toBeTruthy()
  ...
  it('keeps a rejected first prompt engaging instead of returning to the Hero', () => {
    ...
    expect(b.view.queryByText('向未知，发出信号')).toBeNull()
  })
