...
桌面端从实际解包目录解析内置 Office API、原生引擎和 WASM 引擎，包括 Linux 包探测及 worker 资源。Windows 验证会把完整应用复制到独立的临时安装目录，保留所有文件并避开层层嵌套的仓库构建路径。打包会校验最终运行环境清单，并在两个平台上执行 DOCX、XLSX 和 PPTX 转换。
