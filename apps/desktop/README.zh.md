...
桌面端从实际解包目录解析内置 Office API、原生引擎和 WASM 引擎，包括 Linux 包探测及 worker 资源。本地 Windows 发行包将完整应用放在 `resources/app`，使 Office 原生辅助进程直接读取普通包路径。打包会校验最终运行环境清单，并在两个平台上执行 DOCX、XLSX 和 PPTX 转换。
